...
public sealed class PartitionExtender(IDiskService diskService, ILogger? logger = null)
{
    ...
    public async Task<PartitionExpandResult> TryExpandLastAsync(int diskNumber, CancellationToken ct = default)
    {
        ...
        var lastAfter = after?.Partitions.FirstOrDefault(p => p.Number == last.Number);
        long afterLen = lastAfter?.LengthBytes ?? beforeLen;

        if (afterLen > beforeLen + 1_000_000) // 1MB 넘게 커졌으면 성공
        {
            _logger.LogInformation(
                "마지막 파티션 확장: {Before} → {After}.",
                SizeFormatter.Format(beforeLen), SizeFormatter.Format(afterLen));
            return new(true, true,
                $"마지막 파티션을 {SizeFormatter.Format(beforeLen)} → {SizeFormatter.Format(afterLen)}로 확장했습니다.");
        }

        _logger.LogWarning("마지막 파티션이 확장되지 않았습니다({Before}).", SizeFormatter.Format(beforeLen));
